...
export default function ContentItem(item) {
    ...
    return (
        ...
        <VStack key={item.shortened_url} spacing='24px' pb={2} align='stretch' >
              <Box  borderRadius={7}  boxShadow={'lg'} borderLeftWidth={7} borderLeftColor={!isOpen ? "white" : "pink.300"} bg={!isOpen ? "white" : "gray.50"} 
                    _hover={{
                    textDecoration: 'none',
                ...
                <Grid templateColumns='repeat(12, 1fr)' gap={3} pb={1} pt={1} >
                  ...
                  <GridItem  colStart={3} colEnd={11} overflow="hidden" textAlign="left">{item.content_url} </GridItem>
                  <GridItem colStart={12} colEnd={14} h='6' > 10886 <br/>clicks</GridItem>
                </Grid>
                <HStack>
                  <ButtonGroup size='sm' isAttached variant='outline' ml={3} mb={3} >
                    <Button  bg='pink.50' mr='-px' w={40} overflow="hidden" onClick={ () => copyToClipboard(item.shortened_url)}
                        _hover={{ color: 'white',
                        bg: 'pink.300',
                }}>{domain}/{item.shortened_url}</Button>
                    <Button bg='pink.50' mr='-px' _hover={{ color: 'white',
                  bg: 'pink.300',
                }}>Visit</Button>
                    <Button bg='pink.50' mr='-px' onClick={onToggle} _hover={{ color: 'white',
                  bg: 'pink.300',
                }}>Edit</Button>
      ...
      <Stack
        ...
        <FormControl id="userName" isRequired>
          ...
          <Input bg={useColorModeValue('white', 'gray.700')}
