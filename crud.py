...
def get_content_by_shortened_url(db: Session, shortened_url):
    item: models.Content = db.query(models.Content).filter(models.Content.shortened_url == shortened_url).first()
    if item:
        if item.active == True:
            return item
    else: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="item not found, check link")  

def update_item(item_id: str, item: schemas.ContentUpdate, db: Session, user: schemas.UserRead):
    stored_item: models.Content = db.query(models.Content).filter(models.Content.id == item_id).first()
    if stored_item.owner_id != user.id: #checking if current user is item owner
           raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")
    for var, value in vars(item).items():
        setattr(stored_item, var, value) if value or str(value) == 'False' else None
    db.add(stored_item)
    db.commit()
